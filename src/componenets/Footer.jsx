        ...
        <>
            <footer className='bg-gradient-to-b from-emerald-400 to-green-300 pt-24 pb-7'>
                <div className=' grid items-center grid-cols-3 justify-items-center'>
                    ...
                    {links?.map((list, i) => (
                        <ul key={i} className='grid itc justify-items-center gap-1'>
                            {list?.map((val, i) => (
                                <li className='font-medium text-sm sm:text-xs' key={i}>{val.link}</li>
                            ))}
                        </ul>
                    ))}
                </div>
                <div className='w-7/12 lg:w-[95vw] m-auto mt-9'>
                    <div className='h-[0.3vh] bg-black/30 my-7 md:my-5'>

                    </div>
                    <div className='flex items-center justify-between px-7 md:px-0 md:gap-5 md:flex-col-reverse'>
                        <p className='text-sm md:text-center'>Copyright <sup className='text-base font-bold'>&copy;</sup>All Reserved 2023 <span className='font-semibold'>Travigo</span> Devloped by <span className='font-bold'>Kiran Sahu.</span></p>
